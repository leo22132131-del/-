...
st.title("長照居家服務核對系統（修正 dmaker 刷卡次數版）")
...
dmaker_mode = st.radio(
    "請選擇 dmaker 刷卡紀錄方式：",
    ["按『簽到筆數/次數』計算 (推薦：解決簽到退重複算2次的問題)", "按『數量欄位加總』計算"],
    index=0
)
...
if file_支審 and file_fa300 and file_dmaker:
    try:
        ...
        if "按『簽到筆數/次數』計算" in dmaker_mode:
        ...
        result = merged.rename(columns={'name': '個案姓名', 'date': '服務日期', 'code': '服務代碼', 'pay_type': '費用類別'})
        ...
        if len(diff) == 0:
            ...
            st.warning(f"⚠️ 修正 dmaker 邏輯後，真實異常筆數為：{len(diff)} 筆")
            st.dataframe(diff, use_container_width=True)

            @st.cache_data
            def convert_df(df): return df.to_csv(index=False).encode('utf-8-sig')

            st.download_button("📥 下載精準異常明細 (CSV)", convert_df(diff), "精準核對異常明細.csv", "text/csv")
