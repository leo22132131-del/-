...
if submit_btn:
    ...
    if not has_支審:
        ...
        df2["code"] = df2["服務項目"].apply(get_exact_code)

        df3["name"] = df3["客戶名"].astype(str).str.strip().str.replace("鳯", "鳳")
        df3["code"] = df3["品名"].apply(get_exact_code)
        ...
        final["總數異常"] = (final["支審次數"] != final["dmaker_公自費合計"]) & (~final["code"].str.startswith("QA"))
