...
if (file_支審_日照 or file_支審_居家) and file_FA300 and file_dmaker:
    ...
    # 2. 資料清洗與代碼正規化 (去除尾綴如 -1, -2)
    def clean_code(text):
        if pd.isna(text): return ""
        s = str(text).strip()
        s = re.sub(r'-\d+$', '', s)  # 清除 -1 尾綴
        return s
    ...
    # 在長照服務中，dmaker 標「公費」或「部分負擔」皆屬於政府申報服務範疇
    ...
    if len(diff_df) == 0:
        ...
        st.warning(f"⚠️ 發現 {len(diff_df)} 筆項目不吻合（已修正部分負擔與次碼誤報）：")
